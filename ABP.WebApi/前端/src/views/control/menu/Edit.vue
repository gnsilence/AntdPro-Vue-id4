      ...
      <a-form :form="form" >
        ...
        <a-form-item label="菜单图标" v-bind="layout">
          <v-iconbox v-model="icon" selectType="name">
            <a-input v-model="icon" placeholder="请输入菜单的图标 " style="width: 40%"/>
          </v-iconbox>
          <!-- <a-input placeholder="请输入菜单的图标 " v-decorator="formDecorator.icon" @change="e=>{this.iconType=e.target.value}" style="width: 40%"/> -->
          <a-icon :type="icon" style="font-size: 20px; margin-left: 5px;"></a-icon>
        </a-form-item>
      ...
      this.form.validateFields((err, values) => {
        ...
        modifyMenu(values).then(res => {
          ...
          if (res.result.status === 200) {
            ...
            this.$refs.modal.error('修改菜单失败', res.result.message)
          }
        }).catch(() => { this.close() })
      })
    }
