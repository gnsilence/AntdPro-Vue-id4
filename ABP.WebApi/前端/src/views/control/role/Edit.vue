    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="选择角色" v-bind="layout">
          <a-select disabled placeholder="请给此管理员赋予角色" :allowClear="true" v-decorator="formDecorator.name">
  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        ...
        modifyRole(values).then(res => {
          ...
          if (res.result.status === 200) {
            ...
            this.$error({ title: '修改角色失败', content: res.result.message })
          }
        }).catch(() => { this.onClose() })
      })
    },
    ...
    loadData (id) {
      ...
      getRoleDetail(id).then(res => {
        if (res.result.status === 200) {
          this.defaultPermissions = Object.keys(res.result.result.permissions)
          this.$refs.assign.loadData()
          this.$nextTick(() => {
            this.form.setFieldsValue(pick(res.result.result, 'description', 'enable', 'name'))
          })
        } else {
          this.$notification.error({ message: '获取管理账户详情失败', description: res.result.message })
        }
        this.confirmLoading = false
      }).catch(() => { this.onClose() })
    }
