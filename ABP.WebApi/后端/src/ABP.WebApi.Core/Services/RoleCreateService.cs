...
namespace ASF.Domain.Services
{
    ...
    public class RoleCreateService
    {
        ...
        /// <returns></returns>
        public Result<Role> Create(string name,string roleid, string description, string createOfAccountId)
        {
            //创建管理员不能为空
            if (createOfAccountId ==null)
                return Result<Role>.ReFailure(ResultCodes.RoleCreateFailed);
            var id = DateTime.Now.ToUnixTime32();
            var role = new Role(id,name,roleid, description, createOfAccountId);
            //验证角色聚合的数据合法性
        ...
        /// <returns></returns>
        public async Task<Result<Role>> Create(string name, string roleid, string description, string createOfAccountId, List<string> pids)
        {
            var result = this.Create(name,roleid, description, createOfAccountId);
            if (!result.Success)
                return result;
