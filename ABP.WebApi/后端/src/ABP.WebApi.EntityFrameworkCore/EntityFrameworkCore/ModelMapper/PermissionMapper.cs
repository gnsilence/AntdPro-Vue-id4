    ...
    public class PermissionMapper : Profile
    {
        public PermissionMapper()
        {
            base.CreateMap<Model.PermissionModel, ASF.Domain.Entities.Permission>()
                .ForMember(m => m.HttpMethods, s => s.MapFrom(f => f.HttpMethods.Split(',',System.StringSplitOptions.None).Where(t => !string.IsNullOrEmpty(t)).Select(t => new HttpMethod(t)).ToList()));
