    ...
    public class AccountMapper : Profile
    {
        public AccountMapper()
        {
            base.CreateMap<AccountModel, Account>(MemberList.None)
                ...
                .ForMember(a => a.Roles, model => model.MapFrom(d => d.Roles.Split(',',StringSplitOptions.RemoveEmptyEntries).Select<string, int>(q => string.IsNullOrEmpty(q) ? 0 : Convert.ToInt32(q)).ToList()));
